...
class Sproto {
            ...
            if (args.tagname != self.array_tag) {
                ...
                if (map || args.mainkey != null) {
                    ...
                    if (this._isMap(self.iter_table) || this._isArray(self.iter_table)) {
                        ...
                        throw new Error(
                            `${self.st.name}.${args.tagname} is not a Map or Array, is a ${this._getObjectType(self.iter_table)}`
                        );
                    }
                }
            }
    ...
    _sprotoDecode(st, data, size, cb, ud) {
        ...
        size -= SIZEOF_HEADER;
        if (size < SIZEOF_FIELD * fn) return -1;
        ...
        for (let i = 0; i < fn; i++) {
            ...
            if (value < 0) {
                if (size < SIZEOF_LENGTH) return -1;
                let sz = this._toword(datastream);
                if (size < sz + SIZEOF_LENGTH) return -1;
