...
const mul_mod_p = (a: bigint, b: bigint): bigint => {
    ...
    while (b > 0n) {
        ...
        a = a >= DH_PRIME - a ? a * 2n - DH_PRIME : a * 2n;
        b >>= 1n;
    }
    return m % DH_PRIME;
};
...
const DES_ROUND = (X: number, Y: number, sk1: number, sk2: number): [number, number] => {
    ...
    Y = (Y ^ (SB7[T & 0x3f] ^ SB5[(T >>> 8) & 0x3f] ^ SB3[(T >>> 16) & 0x3f] ^ SB1[(T >>> 24) & 0x3f])) >>> 0;

    return [X, Y]; // 输出强制无符号
};

const LHs: number[] = [
    0x00000000, 0x00000001, 0x00000100, 0x00000101, 0x00010000, 0x00010001, 0x00010100, 0x00010101, 0x01000000, 0x01000001, 0x01000100,
    0x01000101, 0x01010000, 0x01010001, 0x01010100, 0x01010101,
];

const RHs: number[] = [
    0x00000000, 0x01000000, 0x00010000, 0x01010000, 0x00000100, 0x01000100, 0x00010100, 0x01010100, 0x00000001, 0x01000001, 0x00010001,
    0x01010001, 0x00000101, 0x01000101, 0x00010101, 0x01010101,
];
...
export const SpinePlay = (
    ...
    animation: string,
    loop: boolean = false,
    complete: ((x: sp.spine.TrackEntry) => void) | null = null
) => {
    if (!sp) return;
    const skeleton = sp.content as sp.Skeleton;
    if (!skeleton) return;
