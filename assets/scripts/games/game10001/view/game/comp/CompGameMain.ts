...
import { ENUM_POP_MESSAGE_TYPE, MAIN_GAME_ID, RICH_TYPE } from "../../../../../datacenter/InterfaceConfig";
...
} from "../../../../../../types/protocol/game10001/c2s";
...
export class CompGameMain extends FGUICompGameMain {
    ...
    onBtnChange(): void {
        GameSocketManager.instance.sendToServer(SprotoOutHand, {
            ...
            flag: HAND_INDEX[this.ctrl_select.selectedIndex],
        });
    }
    ...
    async drawInviteInfo(): Promise<string> {
        return new Promise<string>(async (resolve, reject) => {
            ...
            canvasContext.fillText(
                TruncateString(DataCenter.instance.userData?.nickname || "", 8),
        ...
        this.drawInviteInfo()
            .then((res: string) => {
                MiniGameUtils.instance.shareAppMessage({
                    ...
                    imageUrl: res,
                    query: `gameid=${MAIN_GAME_ID}&roomid=${DataCenter.instance.shortRoomid}`,
                });
            })
            .catch((err: any) => {
                Logger.log(err);
